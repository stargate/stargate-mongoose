...
describe('tables vector search', function () {
    ...
    const Vector = mongooseFixtures_1.mongooseInstance.model('VectorTable', vectorSchema, 'vector');
    before(async function () {
        ...
        await mongooseFixtures_1.mongooseInstance.connection.createTable('vector', {
            primaryKey: '_id',
            columns: {
                _id: {
                    type: 'text'
        ...
        await mongooseFixtures_1.mongooseInstance.connection.collection('vector').runCommand({
            createVectorIndex: {
                name: 'vectortables',
                definition: {
                    column: 'vector'
    ...
    it('supports sort() with $meta with find()', async function () {
        ...
        await assert_1.default.rejects(Vector.find().limit(1001).sort({ vector: { $meta: [99, 1] } }), /Use of ANN OF in an ORDER BY clause requires a LIMIT that is not greater than 1000/);
    });
