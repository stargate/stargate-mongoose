...
describe('Driver based tests', async () => {
    ...
    describe('Mongoose API', () => {
        ...
        before(async function () {
            const tables = await mongooseFixtures_1.mongooseInstance.connection.listTables();
            const tableNames = tables.map(t => t.name);
            if (process.env.DATA_API_TABLES) {
                await mongooseFixtures_1.mongooseInstance.connection.dropTable(fixtures_1.TEST_COLLECTION_NAME);
                await mongooseFixtures_1.mongooseInstance.connection.createTable(fixtures_1.TEST_COLLECTION_NAME, {
                    primaryKey: '_id',
                    columns: {
                        _id: { type: 'text' },
                        __v: { type: 'int' },
        ...
        it('handles reconnecting after disconnecting', async () => {
            ...
            if (process.env.DATA_API_TABLES) {
                ...
                if (!tableNames.includes(fixtures_1.TEST_COLLECTION_NAME)) {
                    await mongooseInstance.connection.createTable(fixtures_1.TEST_COLLECTION_NAME, {
                        primaryKey: '_id',
                        columns: {
                            _id: { type: 'text' },
                            name: { type: 'text' }
