...
describe('Mongoose Model API level tests', async () => {
    ...
    describe('Options & Data type tests', () => {
        ...
        it('Data type tests', async function () {
            ...
            const collectionNames = await mongooseFixtures_1.mongooseInstance.connection.listCollections({ nameOnly: true });
    ...
    describe('API tests', () => {
        ...
        it('API ops tests Model.cleanIndexes()', async function () {
            if (process.env.DATA_API_TABLES) {
                ...
                // @ts-expect-error
                mongooseFixtures_1.Product.schema._indexes = [];
                ...
                const droppedIndexes = await mongooseFixtures_1.Product.cleanIndexes();
        ...
        it('API ops tests Model.replaceOne()', async function () {
            if (process.env.DATA_API_TABLES) {
                await assert_1.default.rejects(mongooseFixtures_1.Product.replaceOne({ category: 'cat 1' }, { name: 'Product 4' }), /Cannot use replaceOne\(\) with tables/);
                return;
            }
            const product1 = new mongooseFixtures_1.Product({ name: 'Product 1', price: 10, isCertified: true, category: 'cat 2' });
            const product2 = new mongooseFixtures_1.Product({ name: 'Product 2', price: 10, isCertified: true, category: 'cat 2' });
            const product3 = new mongooseFixtures_1.Product({ name: 'Product 3', price: 10, isCertified: true, category: 'cat 1' });
            await mongooseFixtures_1.Product.insertMany([product1, product2, product3]);
            const resp = await mongooseFixtures_1.Product.replaceOne({ category: 'cat 1' }, { name: 'Product 4' });
            assert_1.default.equal(resp.modifiedCount, 1);
            const doc = await mongooseFixtures_1.Product.findOne({ name: 'Product 4' });
            assert_1.default.ok(doc);
            assert_1.default.strictEqual(doc.category, undefined);
            await mongooseFixtures_1.Product.replaceOne({ category: 'cat 2' }, { name: 'Product 5', category: 'cat 3' }, { sort: { name: 1 } });
            const cat2 = await mongooseFixtures_1.Product.findOne({ category: 'cat 2' }).orFail();
            assert_1.default.equal(cat2.name, 'Product 2');
            const replaced = await mongooseFixtures_1.Product.findOne({ name: 'Product 5' }).orFail();
        ...
        it('API ops tests Model.syncIndexes()', async () => {
            if (process.env.DATA_API_TABLES) {
                ...
                const testProductSchema = new mongooseFixtures_1.mongooseInstance.Schema({
                ...
                const ProductIndexModel = mongooseFixtures_1.mongooseInstance.model('ProductIndexes', testProductSchema, mongooseFixtures_1.Product.collection.collectionName);
        ...
        it('API ops tests Model.db.collection()', async () => {
            const product1 = new mongooseFixtures_1.Product({ name: 'Product 1', price: 10, isCertified: true, category: 'cat 2' });
            await product1.save();
            const res = await mongooseFixtures_1.mongooseInstance.connection.collection('products').findOne({});
        ...
        it('API ops tests collection.runCommand()', async function () {
            const res = await mongooseFixtures_1.mongooseInstance.connection.collection('carts').runCommand({ find: {} });
    ...
    describe('vector search', function () {
        ...
        before(async function () {
            if (process.env.DATA_API_TABLES) {
                ...
                const collections = await mongooseFixtures_1.mongooseInstance.connection.listCollections({ nameOnly: false });
