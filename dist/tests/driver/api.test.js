...
describe('Mongoose Model API level tests', async () => {
    ...
    describe('API tests', () => {
        ...
        it('API ops tests Model.cleanIndexes()', async () => {
            ...
            assert_1.default.strictEqual(error.message, 'listIndexes() Not Implemented');
        });
        ...
        it('API ops tests Model.deleteMany()', async () => {
            const product1 = new mongooseFixtures_1.Product({ name: 'Product 1', price: 10, isCertified: true, category: 'cat 1' });
            await product1.save();
            const deleteManyResp = await mongooseFixtures_1.Product.deleteMany({ name: 'Product 1' });
        ...
        it('API ops tests Model.deleteOne()', async () => {
            const product1 = new mongooseFixtures_1.Product({ name: 'Product 1', price: 10, isCertified: true, category: 'cat 1' });
            await product1.save();
            const deleteOneResp = await mongooseFixtures_1.Product.deleteOne({ name: 'Product 1' });
        ...
        it('API ops tests Query cursor', async () => {
            await mongooseFixtures_1.Product.create(Array.from({ length: 25 }, (_, index) => ({
                name: `Product ${index + 1}`,
                price: 10
            })));
            let cursor = mongooseFixtures_1.Product.find().sort({ product: 1 }).cursor();
            for (let i = 0; i < 20; ++i) {
                const product = await cursor.next();
                assert_1.default.equal(product?.name, `Product ${i + 1}`, 'Failed at index ' + i);
            }
            assert_1.default.equal(await cursor.next(), null);
            cursor = await mongooseFixtures_1.Product.find().sort({ product: 1 }).limit(20).cursor();
            for (let i = 0; i < 20; ++i) {
                await cursor.next();
            }
            assert_1.default.equal(await cursor.next(), null);
        ...
        it('API ops tests connection.runCommand()', async () => {
            const connection = mongooseFixtures_1.mongooseInstance.connection;
    ...
    describe('vector search', function () {
        ...
        it('supports sort() with includeSortVector in find()', async function () {
            ...
            assert_1.default.deepStrictEqual(await rawCursor.getSortVector(), [1, 99]);
        });
        ...
        it('contains vector options in listCollections() output with `explain`', async function () {
            ...
            assert_1.default.deepStrictEqual(collection.options, {
