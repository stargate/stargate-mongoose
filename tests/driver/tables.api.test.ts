...
describe('TABLES: Mongoose Model API level tests', async () => {
    ...
    describe('Options & Data type tests', () => {
        it('test strict and strictQuery options', async () => {
            ...
            // @ts-expect-error extraCol is not defined in the schema
    ...
    describe('API tests', () => {
        ...
        it('API ops tests Model.cleanIndexes()', async function() {
            ...
            // @ts-expect-error hack to clear the schema indexes for testing
        ...
        it('API ops tests Model.discriminator()', async () => {
            ...
            // @ts-expect-error url is not defined in schema
        ...
        it('API ops tests useDb() with isTable option', async function() {
            const connection = mongooseInstance.createConnection(testClient!.uri, { ...testClient!.options, isTable: true }) as unknown as AstraMongooseDriver.Connection;
            await connection.asPromise();
            const { keyspaceName } = parseUri(testClient!.uri);
